...
pip_uninstall() {
    "${PYTHON_BIN}" -m pip uninstall "$@"
}

echo "1. install inference frameworks and pytorch they need"
if [ $USE_SGLANG -eq 1 ]; then
    pip_install "sglang[all]==0.5.2" --no-cache-dir
...
pip_uninstall -y pynvml nvidia-ml-py || true
...
if [ $USE_MEGATRON -eq 1 ]; then
    ...
    NVTE_FRAMEWORK=pytorch "${PYTHON_BIN}" -m pip install --no-deps git+https://github.com/NVIDIA/TransformerEngine.git@v2.6
    "${PYTHON_BIN}" -m pip install --no-deps git+https://github.com/NVIDIA/Megatron-LM.git@core_v0.13.1
fi
...
pip_install opencv-fixer && \
    "${PYTHON_BIN}" -c "from opencv_fixer import AutoFix; AutoFix()"
